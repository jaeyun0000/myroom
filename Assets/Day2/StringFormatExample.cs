...
public class StringFormatExample : MonoBehaviour
{
    void Start()
    {
        ...
        // 나의 이름은 000이고, 나이는 00살이다. 성별은 00입니다
        
        // 1. + 연산자를 이용한 문자열 더하기 방식
        Debug.Log("나의 이름은 " + name + "이고, 나이는 " + age + "살 입니다. 성별은 " + (isMan ? "남자" : "여지") + "입니다.");
        
        // 2. 문자열 서식(String.Format)을 이용한 방식
        string gender = isMan ? "남자" : "여지";
        string introduceString = string.Format("나의 이름은 {0}이고, 나이는 {1}살이다. 성별은 {2}입니다", name, age, gender);
        Debug.Log(introduceString);
        
        // 3. $ 기호를 이용한 문자열 보간
        string introduceString2 = $"나의 이름은 {name}이고, 나이는 {age}살이다. 성별은 {gender}입니다";
